      ...
      <div className='flex flex-col lg:flex-row mt-8 justify-between xl:pr-10'>
        <div className='flex-col gap-y-4'>
          {cartItems?.map((el, index) => (
            <CourseCard key={index} id={el} cartItems={cartItems} setCartItems={setCartItems} />
          ))}
        </div>
  ...
  return (
    <div className='w-full sm:w-[500px] sm:h-[140px] bg-white dropdown-shadow flex flex-col items-center py-3 sm:py-0 sm:flex-row px-10 sm:px-4 mb-8'>
      <div className='h-full flex items-center justify-start'>
        <img src={offer.src} alt='' className='min-w-[85px] min-h-[85px] max-w-[85px] max-h-[85px] rounded-full ' />
      </div>
      <div className='flex flex-col items-center sm:items-start justify-center sm:ml-3 w-[350px]'>
        ...
        <p className='text-[11px] text-center sm:text-left text-gray-400 '>
          By Introduction to Hydroponics Introduction to Hydroponics Introduction to Hydroponics
        </p>
  ...
  return (
    <div className='w-[300px] h-[350px] border shadow-lg bg-white px-5 py-10'>
      ...
      <div className='w-full h-[35px] border rounded-md flex pl-2 mt-6'>
        ...
        <div className='w-[20%] bg-darkBlue centerFlex text-[12px] rounded-r-md text-white '>Apply</div>
      </div>
      ...
      <div className='centerFlex mt-6'>
        <div className='w-[120px] h-[35px] centerFlex text-[13px] rounded-md bg-bizpotta-green'>Continue</div>
      </div>
    </div>
  );
};
