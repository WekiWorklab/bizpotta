...
const BlueFooter = () => {
  ...
  return (
    <div className='w-full  flex flex-col md:translate-x-[200px] md:w-[calc(100%-200px)] justify-end text-white '>
        <div className='px-2 md:px-32 bg-darkBlue py-12 '>
                ...
                <div className='flex flex-col items-center gap-y-4 md:gap-y-8  '>
                    ...
                    <div className='flex flex-col gap-y-2 md:gap-y-6 font-medium text-[13px] md:text-base text-center'>
                        {navData.Bizpotta.map((el, index) => (
                            <p className='' key = {index}>{el.name}</p>
                        ))}
                    </div>
                </div>
        ...
        <div className='w-full py-4 px-4 md:px-16 bg-[#282828]  flex flex-row items-center justify-between'>
            <div className='flex flex-row items-center text-white gap-x-4'>
                ...
                <p className='text-[13px] font-bold hidden md:block'>Figure based on direct platform sales plus estimated course salesvia off platform payment processors. Listed in USD</p>
            </div>
...
export default function BlueFooter() {
  ...
  return (
    <footer className='bg-darkBlue text-white w-full md:translate-x-[200px] md:w-[calc(100%-200px)]' aria-labelledby='footerHeading'>
      ...
      <div className='mt-12 bg-gray-200  py-8 flex justify-between px-10 pt-8 md:flex md:items-center md:justify-between'>
        <div className='flex space-x-6 md:order-2'>
          {footerNavigation.social.map((item) => (
            <a key={item.name} href={item.href} className='text-darkBlue hover:text-darkBlue'>
              <span className='sr-only'>{item.name}</span>
              <item.icon className='h-6 w-6' aria-hidden='true' />
            </a>
          ))}
        </div>
        <p className='mt-8 text-base text-gray-900 md:mt-0 md:order-1'>&copy; 2020 Workflow, Inc. All rights reserved.</p>
      </div>
    </footer>
  );
}
