                ...
                {data?.course?.course_weeks.map((el, index) => (
                    <div key={index} className='flex'>
                        <div className="flex flex-col justify-center items-center">
                            ...
                            <p className="min-w-[50px] text-center text-[11px] mt-2">Week {el.week_number}</p>
                        </div>
                        {(index == data?.course?.course_weeks.length - 1) ? <div/> : <div className="w-16 mt-[10px] border-t-2 border-dashed border-darkBlue "/>}
                    </div>
                ))}
            </div>
        </div>
