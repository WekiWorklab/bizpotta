...
export function LinkCell({ value, row }) {
  return (
    <div className="flex items-center">
      <Link href={`${value}`}>
        <a className=" underline text-takke-green font-semibold">
          View Details
        </a>
      </Link>
    </div>
  ...
  return (
    <span
      className={classNames(
        ...
        status.startsWith("active" || "yes")
          ? "px-2 inline-flex text-xs leading-5 font-semibold rounded-lg bg-[#2DE200]/40 text-green-800"
          : null,
        status.startsWith("inactive" || "no")
          ? "px-2 inline-flex text-xs leading-5 font-semibold rounded-lg bg-[#FC4A4A]/30  text-[#B40303]"
          : null
      )}
    >
      {status}
    ...
    <span
      className={classNames(
        ...
        status.startsWith("yes")
          ? "px-2 inline-flex text-xs leading-5 font-semibold rounded-lg bg-[#6EFF7E] text-green-800"
          : null,
        status.startsWith("no")
          ? "px-2 inline-flex text-xs leading-5 font-semibold rounded-lg bg-[#FDE8E8] text-red-800"
          : null
      )}
    >
      {status}
          ...
          >
            ...
            {showFilter && (
              <div className="absolute top-[40px] w-[180px] py-4 gap-y-2 flex flex-col justify-center items-start dropdown-shadow bg-white rounded-md">
                <p className="w-full text-left px-3 py-2 cursor-pointer text-[13px] hover:bg-[#858585]">
                  All earning
                </p>
                <p className="w-full text-left px-3 py-2 cursor-pointer text-[13px] hover:bg-[#858585]">
                  Last month earning
              ...
              <div className="  ">
                <div className="shadow overflow-hidden border-b border-[#F0F1EB]">
                  ...
                  >
                    ...
                    <tbody
                      {...getTableBodyProps()}
                      className="divide-y divide-[#F0F1EB] "
                    >
                      {page.map((row, i) => {
                        // new
                        prepareRow(row);
