...
const NotesSuccessModal = ({error, setError, success, setSuccess, value, setCurrentValue, section, initialState, setInitialState}) => {
  ...
  return (
    <Transition.Root show={success} as={Fragment}>
      <Dialog as="div" static className="fixed z-10 inset-0 overflow-y-auto" open={success} onClose={closeModal}>
        <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center block sm:p-0  ">
          ...
          >
            <div className="inline-block align-top bg-white rounded-lg overflow-hidden shadow-xl transform transition-all">
              <div className='w-[500px] h-[230px] flex flex-col items-center justify-center'>
           
                  <TextEditorNotesModal api_key = {API_KEY} value = {value} setCurrentValue = {setCurrentValue} section = {section}  initialState = {initialState} setInitialState = {setInitialState}/>
